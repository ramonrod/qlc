...
def main(argv):
    ...
    for b in quanthistling.dictdata.books.list:
        ...
        if book:
            print "Exporting heads with translations for %s..." % b['bibtex_key']
            for dictdata in book.dictdata:
                ...
                if len(heads) > 0:
                    ...
                    for i in range(0,len(heads)):
                        head = heads[i].string.strip()
                        translations = model.meta.Session.query(model.Annotation).filter_by(entry_id=heads[i].entry.id, value=u"translation").all()
                        ...
                        for t in translations:
                            file_heads.write(head.encode('utf-8') + "\t" + t.string.strip().encode("utf-8") + "\thttp://cidles.eu/quanthistling/" + url +"\n")
                    file_heads.close()
